...
public class ApiCollectionUtils {
    ...
    public static <T> List<T> listNonNullOf(final @Nullable List<T> list) {
    ...
    public static <K, V> Map<K, V> mapOf1(@Nullable final Collection<V> list, @NotNull final Function<V, K> keyProvider) {
        return mapOf(list, keyProvider);
    }
     ...
     */
    @NotNull
    public static <T> List<T> inLeftOnly(@Nullable final List<T> left, @Nullable final List<T> right) {
        Set<T> leftSet = Sets.newHashSet(emptyListIfNull(left));
        Set<T> rightSet = Sets.newHashSet(emptyListIfNull(right));
